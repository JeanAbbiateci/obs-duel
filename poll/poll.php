...
function show_vote_control($poll_id) {
	...
	if($poll->control_type === $CONTROL_RADIOBUTTONS) {
		...
		foreach ($poll->values as $value_id => $description) {
			...
			echo("\t\t<input type=\"radio\" name=\"" . htmlspecialchars($VOTE_PARAM_NAME) .
				"\" value=\"" . htmlspecialchars($value_id) . "\"" . "id=\"id" . htmlspecialchars($value_id) . "\"/>");
			if( isset($poll->urls[$value_id]) && !empty($poll->urls[$value_id]) ) {
				...
				echo("<label for=\"id" . htmlspecialchars($value_id) . "\">" . $description . "</label>");
			}
			echo("</div>");
		}
	...
	echo("\t\t<div class=\"container_submit transition_collapse_vertical\"><div><input type=\"submit\" value=\"" . $SUBMIT_BUTTON_STRING . "\" class=\"submit\" />\r\n");
//	echo("\t</p>\r\n\t<p class=\"currentResults\">\r\n");
	
	// Show results link
	// echo("\t\t<a class=\"currentResults\" href=\"" . $POLL_URL . "results.php?" . htmlspecialchars($POLL_ID_PARAM_NAME) . "=" . htmlspecialchars($poll_id) . "\">" . $VIEW_RESULTS_STRING . "</a>\r\n");
